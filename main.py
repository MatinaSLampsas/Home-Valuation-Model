...
# Load the data
data = pd.read_csv('data/all_repeated_sales.csv')
...
print(data[['dist_ocean','dist_downtown','dist_calpoly']].head())
# Rename the column
#data.rename(columns={'Full_Adress': 'Full_Address'}, inplace=True)
print(data.columns)

data = data.drop(columns= 'dist_coast')

data.to_csv('data/all_repeated_sales.csv', index = False)

# Show updated DataFrame with new distance variables
data[['full_address','dist_ocean', 'dist_downtown', 'dist_calpoly']].head()
print(data[['dist_ocean', 'dist_downtown', 'dist_calpoly']].head())
...
house_geometry = [Point(xy) for xy in zip(data['Latitude'], data['Longitude'])]
